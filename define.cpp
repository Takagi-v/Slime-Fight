...
SkillCallback defaultCb = [](BattleField *field, Actor *me, Skill *skill)
{
  ...
  int damage = int(round(typeRatio.at({skill->type, oppPet.type}) *
                         double(weatherRatio[{Weather_T(field->attr["weather"]), skill->type}]) / 100. * skill->power * myPet.attack / oppPet.defense));
  PRINT(ss, "Damage:", damage);

  if (oppPet.attr["sleep"])
  {
    oppPet.attr["sleep"] = oppPet.attr["sleepCnt"] = 0;
...
SkillCallback SingCb = [](BattleField *field, Actor *me, Skill *skill)
{
  ...
  oppPet.attr["sleep"] = 1;
  oppPet.attr["sleepCnt"] = 3;
  std::string name = (me->opp->GetName() == "You" ? "Your" : me->opp->GetName() + "'s");
  PRINT(ss, name, oppPet.petName, "falls asleep.");
...
SkillCallback PaintCb = [](BattleField *field, Actor *me, Skill *skill)
{
  ...
  bool success = true;
  std::string name = (me->opp->GetName() == "You" ? "Your" : me->opp->GetName() + "'s");
  PRINT(ss, name, oppPet.petName, "is painted.");

  if ((me->opp->skill_success) && (!oppPet.attr["sleep"]))
  {
    return SkillCbReturn{
        .success = false,
        .damage = 0,
...
{
  std::stringstream ss;
  Pet &myPet = me->petOnCourt;
  double heal;
  bool success = true;

  if (me->field->attr["weather"] == Weather::Sunny)
    heal = allPets.at(myPet.pet).health * 0.4;
  else if (me->field->attr["weather"] == Weather::Rainy)
    heal = allPets.at(myPet.pet).health * 0.2;
  else
    heal = allPets.at(myPet.pet).health * 0.3;

  if (myPet.health + heal > allPets.at(myPet.pet).health)
    myPet.health = allPets.at(myPet.pet).health;
  else
    myPet.health = myPet.health + heal;

  PRINT(ss, "Recovery", heal);
...
SkillCallback ThunderCb = [](BattleField *field, Actor *me, Skill *skill)
{
  ...
  int damage = int(round(typeRatio.at({skill->type, oppPet.type}) *
                         double(weatherRatio[{Weather_T(field->attr["weather"]), skill->type}]) / 100. * skill->power * me->petOnCourt.attack / oppPet.defense));
  PRINT(ss, "Damage:", damage);
